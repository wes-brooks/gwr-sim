...
for (k in 1:100) {
    write.log(paste('starting k=', k, sep=''), logfile)
    ...
    write.table(sim, file=paste("simdata", process, k, "csv", sep="."))
    ...
    model = lagr(Y~X1+X2+X3+X4+X5-1, data=sim, family='gaussian', coords=sim[,c('loc.x','loc.y')], longlat=FALSE, varselect.method='AICc', bw=0.15, kernel=epanechnikov, bw.type='knn', verbose=TRUE)
    coefs = t(sapply(model[['model']][['models']], function(x) x[['coef']]))
    write.table(coefs, file=paste("coefs", "lagr", process, k, "csv", sep="."))
    ...
    model = lagr(Y~X1+X2+X3+X4+X5-1, data=sim, family='gaussian', coords=sim[,c('loc.x','loc.y')], longlat=FALSE, oracle=oracle, bw=0.15, kernel=epanechnikov, bw.type='knn', verbose=TRUE)
    coefs = t(sapply(model[['model']][['models']], function(x) x[['coef']]))
    write.table(coefs, file=paste("coefs", "oracle", process, k, "csv", sep="."))
    ...
    model = lagr(Y~X1+X2+X3+X4+X5-1, data=sim, family='gaussian', coords=sim[,c('loc.x','loc.y')], longlat=FALSE, oracle=allvars, bw=0.15, kernel=epanechnikov, bw.type='knn', verbose=TRUE)
    coefs = t(sapply(model[['model']][['models']], function(x) x[['coef']]))
    write.table(coefs, file=paste("coefs", "gwr", process, k, "csv", sep="."))
