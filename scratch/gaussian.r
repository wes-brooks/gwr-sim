...
del.AIC.bw = smooth$y - mean(smooth$y)

#Now restrict our attention to the region of the densest 99% of bandwidth probability mass
maxi = which(cumsum(exp(-0.5 * del.AIC.bw))/sum(exp(-0.5 * del.AIC.bw))>0.995)[1]
mini = tail(which(cumsum(exp(-0.5 * del.AIC.bw))/sum(exp(-0.5 * del.AIC.bw))<0.005),1)
...
bws = xxx[sapply(runif(101), function(x) which(x<pp)[1])]
models = list()
models[[1]] = lagr(Y~X1+X2+X3+X4+X5-1, data=sim, family='gaussian', coords=sim[,c('loc.x','loc.y')], longlat=FALSE, varselect.method='AICc', bw=bw.lagr[['bw']], kernel=epanechnikov, bw.type='knn', simulation=TRUE, verbose=FALSE)
for (bw in bws) {
    indx = sample(1:nrow(sim), replace=TRUE)
    boot = sim[indx,]
    models[[length(models)+1]] = lagr(Y~X1+X2+X3+X4+X5-1, data=boot, family='gaussian', fit.loc=sim[,c('loc.x','loc.y')], coords=boot[,c('loc.x','loc.y')], longlat=FALSE, varselect.method='AICc', bw=bw, kernel=epanechnikov, bw.type='knn', simulation=TRUE, verbose=FALSE)
}
